...
'@
    ...
    # HELP-TEXT-START  (generated from README.md -- see CLAUDE.md)
    #   README.md is the single source of truth; the text below is generated.
    #   Regenerate on Linux (no PowerShell needed):  uv run tools/gen_help.py
    $helpText = @'
==============================
 Switch-Window
==============================

(Generated from README.md -- the single source of truth.)
...
------------------------------
  ...
  version:  v1
  status:   detached background process, global hotkey, persistent -- done

  version:  v2
  status:   app-group headers, status bar (count + live clock) -- done

  version:  v3
  status:   ! command mode (done), custom labels (done), !quit (done), configurable hotkey (done)
...
'@
    ...
    $runCommand = {
        ...
        switch ($parts[0].ToLower()) {
            ...
            '!help' {
                # Write the (CRLF-normalised) help text to a temp file and open
                # it in Notepad -- far more readable than a cramped MessageBox.
                $helpPath = Join-Path $env:TEMP 'pwsh-switch-window-help.txt'
